...
FROM
  (SELECT
   ...
   FROM patient p
     ...
     LEFT JOIN concept_reference_term_map_view dcn ON dcn.concept_id = drug_order.dose_units and dcn.concept_reference_source_name='MSF-INTERNAL' and dcn.concept_map_type_name= 'SAME-AS'
     LEFT JOIN concept_name dosecn ON dosecn.concept_id = drug_order.dose_units AND dosecn.concept_name_type = "FULLY_SPECIFIED" AND dosecn.voided = 0
     LEFT JOIN concept_reference_term_map_view rou ON rou.concept_id = drug_order.route and rou.concept_reference_source_name='MSF-INTERNAL' and rou.concept_map_type_name= 'SAME-AS'
     LEFT JOIN concept_name routecn ON routecn.concept_id = drug_order.route AND routecn.concept_name_type = "FULLY_SPECIFIED" AND routecn.voided = 0
